...
from cn_clip.training.new_train import train, evaluate
from cn_clip.training.new_data import get_data
...
def main():
    ...
    if args.classifier_resume is not None:
        if os.path.isfile(args.classifier_resume):
            logging.info(
                f"=> begin to load classifier-checkpoint '{args.classifier_resume}'"
            )
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if args.distllation:
            num_steps_this_epoch = train(model, classifier_model, attn_model, data, epoch, optimizer, scaler, scheduler,
                                         args, steps, teacher_model)
        else:
            num_steps_this_epoch = train(model, classifier_model, attn_model, data, memory_dict, speaker_list, epoch,
                                         optimizer, scaler, scheduler, args, steps)
        ...
        if args.should_save and num_steps_this_epoch > 0:
            if (epoch + 1) == args.max_epochs or (
                ...
                t1 = time.time()
                save_path = os.path.join(args.checkpoint_path, f"epoch{epoch + 1}_{args.type}.pt")
                save_path_classifier = os.path.join(args.checkpoint_path, f"epoch{epoch + 1}_{args.type}_classifier.pt")
                torch.save(
                    {
                        "epoch": epoch + 1,
                        "step": steps,
            ...
            t1 = time.time()
            save_path = os.path.join(args.checkpoint_path, f"epoch_latest_{args.type}.pt")
            torch.save(
                {
                    "epoch": epoch + 1,
                    "step": steps,
            ...
            # Save the latest params
            t1 = time.time()
            save_path = os.path.join(args.checkpoint_path, f"epoch_latest_classifier.pt")
            torch.save(
                {
                    "epoch": epoch + 1,
                    "step": steps,
            ...
            logging.info(
                "Saved checkpoint {} (epoch {} @ {} steps) (writing took {} seconds)".format(save_path, epoch + 1,
                                                                                             steps, time.time() - t1))
        evaluate(model, data, 1, args, classifier_model, is_test=1)
